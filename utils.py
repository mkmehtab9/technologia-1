...
async def get_settings(group_id):
    ...
    if settings is None:  # Use None check to avoid overwriting with default unnecessarily
        settings = await db.get_settings(group_id)
        temp.SETTINGS[group_id] = settings
    return settings
    
async def save_group_settings(group_id, updates):
    settings = await get_settings(group_id)
    settings.update(updates)
    temp.SETTINGS[group_id] = settings  # Update cache
    await db.update_settings(group_id, settings)
    print(f"Saved settings for group {group_id}: {settings}")  # Debug log

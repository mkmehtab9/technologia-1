...
from database.ia_filterdb import av_search_results, get_all_files
...
async def ai_spell_check(chat_id, wrong_name):
    try:  
        ...
        for _ in range(5):
            ...
            files, offset, total_results = await av_search_results(chat_id=chat_id, query=movie)
...
async def requests(client, message):
    ...
    files, offset, total_results = await av_search_results(chat_id=message.chat.id, query=requested_movie)
    ...
    if files: 
        ...
        if closest_movie:
            files, offset, total_results = await av_search_results(chat_id=message.chat.id, query=closest_movie)
            if files:
                file_name = files[0]['file_name']
                await message.reply_text(f"🎥 {file_name}\n\nआपने जो मूवी रिक्वेस्ट की है वो ग्रुप में उपलब्ध हैं\n\nग्रुप लिंक = {GRP_LNK}")
            else:
